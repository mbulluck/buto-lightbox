...
(function(a){var b=function(d,c){return a.ajax({dataType:"jsonp",type:"get",jsonpCallback:"callback"+d+"a",url:c+"/"+d+".jsonp?callback=?"})};a.fn.butoLightbox=function(c){var d=a.extend({api_uri:"//api.buto.tv/v2/video"},c);return this.each(function(){var e=a(this);var f=e.data("id");var h=new Spinner().spin(this);var g=b(f,d.api_uri);g.success(function(j){var i=a("<a>").prop("href","//embed.buto.tv/"+f).prop("title",j.media_title);var k=a("<img>").prop("src",j.uri.poster_frame).prop("alt",j.media_title);if(e.height()===0||e.height()===undefined){k.prop("height",j.published_height).prop("width",j.published_width)}else{k.prop("width",e.width()).prop("height",e.height())}i.html(k);i.colorbox({iframe:true,innerWidth:j.published_width,innerHeight:j.published_height});h.stop();e.append(i)})})}})(jQuery);